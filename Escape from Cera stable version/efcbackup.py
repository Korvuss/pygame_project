...
class Level(object):
    ...
    def tick(self, **kwargs):
		if "force" in kwargs:
			self.players[0].tick(self,force = kwargs["force"])
...
class Rock(object):
    def __init__(self):
        ...
        self.dy = 0
#----------------------------------------------------------------------------------------------------------------------------------
class CordParserP1(threading.Thread):
	def __init__(self):
	...
	def run(self):
		while(True):
			...
			#	print dataPrime
				...
				self.zNew = float(zP)
#----------------------------------------------------------------------------------------------------------------------------------
class CordParserP2(threading.Thread):
	def __init__(self):
		threading.Thread.__init__(self)
		self.xNew = None
		self.zNew = None
	...
	def run(self):
		while(True):
			data = self.sock.readline()
			self.tagIdPlayer2 = data[25:28]
			if self.tagIdPlayer2 == "062":	
				dataPrime = data.split()			
				z = dataPrime[3]
				x = dataPrime[1]
...
zOld = 0.0
move = ""
parser1 = CordParserP1()
parser1.start()
parser2 = CordParserP2()
parser2.start()
xn = 0.0
zn = 0.0
images = {}
